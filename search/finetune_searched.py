...
def main(**kwargs):
    ...
    if train_config.use_peft:
        if train_config.from_peft_checkpoint:
            ...
            with open(train_config.searched_network_file, 'r') as f:
            ...
            if config.model_type == "mllama":
                ...
                for name, module in all_modules:
                    if isinstance(module, LoraLinear):
                        sampled_rank = rank_dict[name]

                        start_idx = (peft_config.r - sampled_rank) // 2
                        end_idx = start_idx + sampled_rank

                        sampled_A = module.lora_A["default"].weight.clone()[start_idx:end_idx, :]
                        sampled_B = module.lora_B["default"].weight.clone()[:, start_idx:end_idx]
                        
                        module.lora_A["default"].weight.data = sampled_A
                        module.lora_B["default"].weight.data = sampled_B 

                        module.lora_A["default"].out_features = sampled_rank
                        module.lora_B["default"].in_features = sampled_rank

                        module.scaling["default"] = module.lora_alpha["default"] / sampled_rank
    ...
    train_dataloader = torch.utils.data.DataLoader(dataset_train, num_workers=train_config.num_workers_dataloader,
                                                   pin_memory=True, **train_dl_kwargs)
