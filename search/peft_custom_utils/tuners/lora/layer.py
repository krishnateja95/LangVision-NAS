...
from peft_custom_utils.utils.integrations import dequantize_bnb_weight, gather_params_ctx, get_bnb_param_type
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_superlayer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, search_space, use_dora: bool = False, lora_bias: bool = False):
        ...
        self.max_r = max(search_space)

        self.r[adapter_name] = self.max_r
        ...
        self.lora_A[adapter_name] = nn.Linear(self.in_features, self.max_r, bias=False)
        ...
        if use_rslora:
            ...
            self.scaling[adapter_name] = lora_alpha / self.max_r
        ...
        self.A_bias = None
        self.B_bias = lora_bias
    
    def update_simplelayer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, search_space, use_dora: bool = False, lora_bias: bool = False):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, search_space, use_dora: bool = False, lora_bias: bool = False):
        ...
        if search_space != []:
            self.update_superlayer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, search_space, use_dora, lora_bias)
        else:
            self.update_simplelayer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora, search_space, use_dora, lora_bias)
...
class Linear(nn.Module, LoraLayer):
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    if self.if_supernet:
                        result = result + self.forward_supernet(x, lora_A, lora_B, scaling, dropout)
                    else:
    ...
    def get_sampled_network(self):
        ...
        sampled_A = self.lora_A["default"].weight.clone()[start_idx:end_idx, :]
        sampled_B = self.lora_B["default"].weight.clone()[:, start_idx:end_idx]
        
        self.lora_A["default"].weight.data = sampled_A
        self.lora_B["default"].weight.data = sampled_B 

        self.lora_A["default"].out_features = sampled_rank
        self.lora_B["default"].in_features = sampled_rank

        if self.use_rslora:
            self.scaling["default"] = self.lora_alpha["default"] / math.sqrt(sampled_rank)
        else:
